...
#include <linux/completion.h> // Para esperar el callback

#define DRIVER_NAME "arduino_usb_driver"
#define DEVICE_NAME "arduino_uno"
...
static void write_urb_complete(struct urb *urb){
    pr_info("arduino_usb_driver: write_urb_complete status=%d actual_length=%d\n",
...
static ssize_t arduino_write(struct file *file, const char __user *buf, size_t count, loff_t *ppos) {
    ...
    if (count > 64)  // limita tamaño (puede ajustarse)
        count = 64;
...
// Identificadores USB para interfaz CDC ACM y CDC Data
...
MODULE_DESCRIPTION("Driver USB simple para Arduino Uno R3 (CDC ACM)");
MODULE_LICENSE("GPL");
